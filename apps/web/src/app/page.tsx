...
const capabilities = [
  {
    ...
    description: 'Tenant-scoped failure records and event timelines.',
    icon: AlertTriangle,
    accent: 'text-red-600',
  },
  {
    title: 'Recommended Remediation',
    description: 'Deterministic suggestions derived from incident evidence.',
    icon: Search,
    accent: 'text-[#ff9900]',
  },
  {
    title: 'Governed Operation Preparation',
    description: 'Safe mapping of recommendations to the approval pipeline.',
    icon: ShieldCheck,
    accent: 'text-emerald-600',
  },
  {
    title: 'Operations Hub',
    description: 'Centralized command center for queues, failures, and approvals.',
    icon: Activity,
    accent: 'text-[#0972d3]',
  },
  {
    title: 'Governance Center',
    description: 'Immutable, audit-ready evidence log of all operations.',
    icon: CheckCircle2,
    accent: 'text-purple-600',
  },
  {
    title: 'Resource Graph',
    description: 'Database-backed read-only topology mapping.',
    icon: Layers,
    accent: 'text-teal-600',
  },
  {
    title: 'Provider Integrations',
    description: 'Connectors for Kubernetes, Docker, AWS, and Jenkins.',
    icon: Server,
    accent: 'text-[#16191f]',
  },
  {
    title: 'Deployments',
    description: 'Tenant-isolated structures defining deployment lifecycles.',
    icon: GitMerge,
        ...
        <div className="mx-auto flex min-h-[72px] max-w-[1600px] items-center justify-between gap-4 px-4 py-3 sm:px-6">
          <div className="flex min-w-0 items-center gap-10">
            <Link href="/" className="flex flex-col items-start gap-0">
              <span className="text-3xl font-extrabold tracking-tight text-[#16191f] leading-none">
                autoops
              </span>
            ...
            <nav className="hidden items-center gap-8 text-[15px] font-semibold text-[#16191f] lg:flex">
              <Link href="#services" className="hover:text-[#0972d3]">
                Services
              </Link>
              <Link href="#solutions" className="hover:text-[#0972d3]">
                Solutions
              </Link>
              <Link href="#governance" className="hover:text-[#0972d3]">
                Governance
              </Link>
              <Link href="#resources" className="hover:text-[#0972d3]">
                Resources
              </Link>
            </nav>
          </div>
          <div className="flex shrink-0 items-center gap-3 text-[15px] font-semibold sm:gap-6">
            ...
            <Link href="/login" className="hidden sm:block text-[#16191f] hover:text-[#0972d3]">
              Sign in to console
            </Link>
            <Link
              href="/register"
              className="rounded-full bg-[#16191f] px-4 py-2.5 text-sm text-white transition-colors hover:bg-[#232f3e] sm:px-6 sm:text-[15px]"
            >
              Create account
            </Link>
          </div>
        ...
        <div className="mx-auto max-w-[1600px] px-6 pt-6">
          ...
          <div className="mb-16 max-w-full overflow-x-auto rounded-xl border border-[#d5dbdb] bg-white px-5 py-5 shadow-sm sm:px-8">
            <nav className="flex w-max min-w-full items-center gap-6 whitespace-nowrap text-[16px] font-bold text-[#16191f] lg:gap-10">
              ...
              <Link href="#overview" className="hover:text-[#0972d3] font-semibold">
                Overview
              </Link>
              <Link href="#services" className="hover:text-[#0972d3] font-semibold">
                Service categories
              </Link>
              <Link href="#governance" className="hover:text-[#0972d3] font-semibold">
                Governed operations
              </Link>
              <Link href="#resources" className="hover:text-[#0972d3] font-semibold">
                Runbooks
              </Link>
            </nav>
          </div>
          ...
          <div id="overview" className="max-w-[900px] py-10">
            <p className="text-[15px] font-bold tracking-wide text-[#0972d3] uppercase mb-4">
              AutoOps
            </p>
            ...
            <p className="mt-6 text-xl sm:text-2xl leading-[1.6] text-[#232f3e] max-w-3xl">
              Discover services, observe health, approve risky actions, respond to incidents, and
              recover failed operations with real backend data and safe controls.
            </p>
            <div className="mt-10 flex flex-wrap items-center gap-4">
              <Link
                href="/login"
                className="rounded-full bg-[#ff9900] px-8 py-4 text-[17px] font-bold text-[#16191f] hover:bg-[#ffb84d] transition-colors flex items-center gap-2"
              >
                Open Demo Console <ArrowRight className="h-5 w-5" />
              </Link>
              <Link
                href="https://github.com/Pramu55/AutoOps-2.0"
                target="_blank"
                className="rounded-full border-2 border-[#16191f] bg-transparent px-8 py-4 text-[17px] font-bold text-[#16191f] hover:bg-[#16191f] hover:text-white transition-colors flex items-center gap-2"
              >
                <Github className="h-5 w-5" /> View GitHub
              </Link>
              <Link
                href="/login"
                className="rounded-full bg-white border border-[#d5dbdb] px-8 py-4 text-[17px] font-bold text-[#16191f] hover:bg-slate-50 transition-colors"
              >
                View Documentation
              </Link>
            </div>
        ...
        <div className="mx-auto max-w-[1600px] px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {capabilities.map(({ title, description, icon: Icon, accent }) => (
              <div
                key={title}
                className="group relative rounded-lg border border-[#eaeded] bg-white p-6 shadow-sm hover:shadow-md hover:border-[#0972d3] transition-all cursor-pointer flex flex-col"
              >
                <div
                  className={`mb-4 flex h-10 w-10 items-center justify-center rounded bg-[#f2f8fd] ${accent}`}
                >
                  <Icon className="h-5 w-5" />
                </div>
                <h3 className="text-lg font-bold text-[#0972d3] group-hover:underline mb-2">
                  {title}
                </h3>
                <p className="text-[15px] text-[#5f6b7a] leading-relaxed flex-grow">
                  {description}
                </p>
              </div>
            ))}
        ...
        <div className="mx-auto max-w-[1600px] px-6">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {integrations.map(({ title, icon: Icon }) => (
              <div
                key={title}
                className="flex items-center gap-3 rounded-lg border border-[#eaeded] bg-white px-6 py-4 shadow-sm hover:border-[#0972d3] transition-colors cursor-pointer"
              >
      ...
      <section id="governance" className="bg-white py-24 border-y border-[#d5dbdb]">
        <div className="mx-auto max-w-[1600px] px-6">
          <div className="grid gap-12 lg:grid-cols-2 items-center">
            <div>
              <p className="text-[15px] font-bold tracking-wide text-[#ff9900] uppercase mb-3">
                Governance Built In
              </p>
              ...
              <p className="mt-6 text-[18px] leading-relaxed text-[#232f3e]">
                AutoOps keeps the console focused on what an operator needs next: status, action
                required, recent failures, incidents, approvals, and audited recovery.
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {safetyPoints.map((point) => (
                <div
                  key={point}
                  className="flex items-center gap-3 rounded border border-[#eaeded] bg-white p-4 shadow-sm"
                >
      ...
      <section id="solutions" className="bg-[#232f3e] py-24 text-white">
        <div className="mx-auto max-w-[1600px] px-6">
          <div className="max-w-3xl mb-12">
            ...
            <p className="mt-4 text-[17px] text-[#eaeded]">
              Follow these steps to evaluate the core mechanics of the platform.
            </p>
          </div>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {demoSteps.map((step, idx) => (
              <div
                key={step}
                className="relative rounded-lg border border-white/10 bg-white/5 p-6 hover:bg-white/10 transition-colors"
              >
        ...
        <div className="mx-auto max-w-[1600px] px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex flex-col items-center md:items-start gap-1">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded bg-[#f2f8fd] px-2 py-0.5 text-[12px] font-bold text-[#0972d3]">
                v1.4.1
              </span>
            </div>
          ...
          <Link
            href="https://github.com/Pramu55/AutoOps-2.0"
            target="_blank"
            className="flex items-center gap-2 text-[14px] font-bold text-[#16191f] hover:text-[#0972d3] transition-colors"
          >
