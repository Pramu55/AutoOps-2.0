  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <section className="rounded-3xl border border-emerald-300/15 bg-emerald-300/10 p-4">
        <div className="flex items-start gap-3 text-sm font-medium text-emerald-200">
      ...
      {currentStatus !== 'CONNECTED' ? (
        <section className="rounded-3xl border border-amber-300/20 bg-amber-300/10 p-6">
          <div className="flex gap-4">
            ...
            <div>
              <h2 className="text-base font-semibold text-white">Docker connector is not available</h2>
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[0.055] p-5 shadow-xl shadow-black/10">
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-base font-semibold text-white">Containers</h2>
        ...
        <div className="mt-5 space-y-3">
          {isLoading ? (
            ...
            containers.map((container) => (
              <article
                key={container.id}
                className="grid gap-4 rounded-2xl border border-white/10 bg-slate-950/35 p-4 xl:grid-cols-[1.1fr_0.55fr_0.6fr_0.65fr_auto]"
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-3 truncate text-sm font-semibold text-white">{container.name}</p>
                  <p className="mt-1 font-mono text-xs text-slate-500">{shortId(container.id)}</p>
                  <p className="mt-2 truncate text-xs text-slate-400">{container.image}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wide text-slate-500">Status</p>
                  <p className="mt-1 text-sm text-slate-300">{container.status}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wide text-slate-500">Ports</p>
                  <p className="mt-1 text-sm text-slate-300">{portLabel(container)}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wide text-slate-500">Created</p>
                  <p className="mt-1 text-sm text-slate-300">{formatTime(container.createdAt)}</p>
                </div>
                  ...
                  {isRunning(container) ? (
                    <>
                      ...
                      <Button
                        ...
                        className="rounded-full border-amber-300/30 bg-amber-300/10 text-amber-100"
                      >
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_0.9fr]">
        <section className="rounded-3xl border border-white/10 bg-white/[0.055] p-5 shadow-xl shadow-black/10">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              <h2 className="text-base font-semibold text-white">Container logs</h2>
              <p className="mt-1 text-sm text-slate-400">Capped logs for the selected container.</p>
            </div>
          ...
          <div className="mt-5">
            {logsError ? (
              ...
              <pre className="max-h-96 overflow-auto rounded-2xl border border-white/10 bg-slate-950/75 p-4 text-xs leading-5 text-slate-200">
                {logs?.lines.join('\n')}
              </pre>
            )}
          </div>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/[0.055] p-5 shadow-xl shadow-black/10">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h2 className="text-base font-semibold text-white">Recent Docker Operations</h2>
              <p className="mt-1 text-sm text-slate-400">Real worker-backed Docker activity.</p>
            </div>
            <span className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1.5 text-xs font-medium text-cyan-200">
              ...
              activity.map((item) => (
                <article key={item.id} className="rounded-2xl border border-white/10 bg-slate-950/35 p-4">
                  ...
                  <div className="mt-4 grid gap-3 text-sm sm:grid-cols-3">
                    <div>
                      <p className="text-xs uppercase tracking-wide text-slate-500">Created</p>
                      <p className="mt-1 text-slate-300">{formatTime(item.createdAt)}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-slate-300">{actorLabel(item.actor)}</p>
                    </div>
                  </div>
                </article>
              ))
