...
export function IncidentDetailClient({ incidentId }: { incidentId: string }) {
  ...
  if (isLoading) {
    return <div className="rounded-lg border border-slate-200 bg-white p-8 text-sm text-slate-700">Loading incident...</div>;
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[0.85fr_1.15fr]">
        <section className="rounded-lg border border-slate-200 bg-white p-5">
          ...
          <div className="mt-5 space-y-3 text-sm">
            <p className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-slate-700">Created {formatDate(incident.createdAt)}</p>
            <p className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-slate-700">Acknowledged by {actorLabel(incident.acknowledgedBy)} at {formatDate(incident.acknowledgedAt)}</p>
            <p className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-slate-700">Resolved by {actorLabel(incident.resolvedBy)} at {formatDate(incident.resolvedAt)}</p>
          </div>
          {incident.permissions.reason ? (
            <p className="mt-4 rounded-xl border border-amber-300/20 bg-amber-300/10 p-3 text-sm text-amber-800">{incident.permissions.reason}</p>
          ) : null}
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5">
          <h2 className="text-base font-semibold text-slate-900">Safe error summary</h2>
          <div className="mt-5 rounded-md border border-rose-400/30 bg-rose-500/10 p-4 text-sm text-rose-800">
          ...
          {incident.resolutionNote ? (
            <div className="mt-4 rounded-md border border-emerald-400/25 bg-emerald-400/10 p-4 text-sm text-emerald-100">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <h2 className="text-base font-semibold text-slate-900">{incident.runbook.title}</h2>
        <p className="mt-1 text-sm text-slate-600">{incident.runbook.summary}</p>
        <div className="mt-5 grid gap-3">
          {incident.runbook.steps.map((step) => (
            <div key={step.order} className="rounded-md border border-slate-200 bg-slate-50 p-4">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full border border-cyan-300/25 bg-cyan-300/10 px-2.5 py-1 text-[11px] font-semibold text-blue-700">{step.actionType}</span>
                <p className="text-sm font-semibold text-slate-900">{step.order}. {step.title}</p>
              </div>
              <p className="mt-2 text-sm text-slate-600">{step.description}</p>
              {step.linkHref && step.linkLabel ? (
                <Link className="mt-3 inline-flex text-sm font-medium text-blue-700 underline decoration-cyan-200/40 underline-offset-4" href={step.linkHref}>{step.linkLabel}</Link>
              ) : null}
            </div>
          ))}
        </div>
          ...
          <div className="w-full max-w-lg rounded-lg border border-slate-200 bg-white p-6 shadow-2xl shadow-slate-900/20">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-xl font-semibold text-slate-900">{modalAction === 'acknowledge' ? 'Acknowledge incident' : 'Resolve incident'}</h2>
              </div>
            ...
            <p className="mt-5 text-sm leading-6 text-slate-700">
              Type <span className="font-semibold text-blue-700">{modalAction === 'acknowledge' ? 'ACKNOWLEDGE' : 'RESOLVE'}</span> to continue.
            </p>
            {modalAction === 'resolve' ? (
              <>
            ...
            {actionError ? <div className="mt-4 rounded-md border border-rose-400/30 bg-rose-500/10 p-3 text-sm text-rose-800">{actionError}</div> : null}
