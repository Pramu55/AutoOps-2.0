...
function Get-RotationEnvironmentValue([string]$Container, [string]$Key) {
  ...
  $process = Start-RotationTrustedDockerProcess @('exec', $Container, 'sh', '-c', $script) 'ENVIRONMENT_INSPECTION_FAILED'
...
function Get-RotationContainerHttpProbe([string]$Container, [int]$Port, [string]$Path, [bool]$Readiness) {
  try {
    $process = Start-RotationTrustedDockerProcess (New-RotationContainerHttpProbeArguments $Container $Port $Path $Readiness) 'CONTAINER_HTTP_PROBE_FAILED'
