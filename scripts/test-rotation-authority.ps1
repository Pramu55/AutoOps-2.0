...
try {
  ...
  $publicRecovery = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'inspect-secret-rotation-recovery.ps1') -Raw
  if ($publicRecovery -match 'AuthorityPlanPath|AuthorityOperationState') { throw 'REQUESTER_RECOVERY_STATE_PARAMETER_PRESENT' }
  if ($publicRecovery -notmatch "Invoke-RotationAuthorityRequest 'GET_RECOVERY_CLASSIFICATION'") { throw 'PUBLIC_RECOVERY_NOT_AUTHORITY_IPC_ONLY' }
  [Console]::WriteLine('PROVISIONING_DEFAULT_NON_MUTATING PASS')
  [Console]::WriteLine('SERVICE_IMAGEPATH_FROM_REPOSITORY_BLOCKED PASS')
  [Console]::WriteLine('REQUESTER_RECOVERY_STATE_PARAMETER_REMOVED PASS')
  [Console]::WriteLine('REQUESTER_PLAN_PARAMETER_REMOVED PASS')
  [Console]::WriteLine('REQUESTER_FAKE_PREPARED_CLASSIFICATION_BLOCKED PASS')
  [Console]::WriteLine('REQUESTER_FAKE_MANUAL_TO_SAFE_RESUME_BLOCKED PASS')
  [Console]::WriteLine('PUBLIC_RECOVERY_USES_AUTHORITY_IPC_ONLY PASS')
