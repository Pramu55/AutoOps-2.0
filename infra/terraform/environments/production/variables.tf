...
variable "environment" {
  description = "Gate 3 production environment name."
  type        = string
  default     = "production"

  validation {
    condition     = var.environment == "production"
    error_message = "environment must equal production for this root module."
  }
}
...
variable "data_classification" {
  description = "Production data classification label."
  type        = string
  default     = "internal"

  validation {
    condition     = contains(["internal", "restricted"], var.data_classification)
    error_message = "data_classification must be internal or restricted for production."
  }
}
